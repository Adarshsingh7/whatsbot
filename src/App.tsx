	...
	const renderContent = () => {
		...
		if (state.completed && state.finalScript) {
			return (
				<div className='space-y-6 animate-in slide-in-from-bottom-8 fade-in duration-700'>
					<div className='flex items-center gap-3 text-green-500 mb-6 justify-center'>
						<CheckCircle2 className='w-6 h-6' />
						<h3 className='text-xl font-semibold text-foreground'>
							Breakthrough Strategy Ready
						</h3>
					</div>

					<div className='p-6 rounded-xl bg-card border shadow-xl ring-1 ring-primary/20'>
						<div className='max-h-[60vh] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-primary/20 scrollbar-track-transparent'>
							<pre className='whitespace-pre-wrap font-sans text-sm md:text-base text-card-foreground leading-relaxed'>
								{typeof state.finalScript === 'string'
									? state.finalScript
									: JSON.stringify(state.finalScript, null, 2)}
							</pre>
						</div>
					</div>
					...
					<Button
						onClick={reset}
						variant='outline'
						className='w-full gap-2 mt-8 py-6 text-lg border-primary/20 hover:bg-primary/5'
					>
		...
		return (
			<div className='space-y-8 min-h-[300px]'>
				{/* Question Area */}
				<div className='space-y-3 animate-in fade-in slide-in-from-right-8 duration-500 min-h-[80px]'>
					<div className='inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary/10 text-primary text-xs font-bold uppercase tracking-wider mb-2'>
						Step {state.step || 1} of 9
					</div>
					{state.question ? (
						...
						<div className='grid grid-cols-1 md:grid-cols-2 gap-3 animate-in fade-in slide-in-from-bottom-4 duration-700 delay-150'>
							{state.suggestions.map((suggestion, idx) => (
								<button
									...
									className='text-left p-4 rounded-xl border border-border/50 bg-muted/30 hover:bg-primary/5 hover:border-primary/30 transition-all group flex items-center justify-between disabled:opacity-50 disabled:cursor-not-allowed h-full'
								>
	...
	return (
		<div className='min-h-screen bg-background text-foreground flex items-center justify-center p-4 md:p-8 selection:bg-primary/30'>
			...
			<div className='fixed inset-0 overflow-hidden pointer-events-none'>
				...
				<div
					className='absolute bottom-[-10%] right-[-10%] w-[40%] h-[40%] rounded-full bg-accent/10 blur-[120px] animate-pulse'
					...
					<CardHeader className='bg-muted/50 border-b border-border/50 px-8 py-6'>
						<div className='flex items-center justify-between'>
							<div className='flex items-center gap-3'>
								<div
									className={`w-2 h-2 rounded-full ${loading ? 'bg-amber-500 animate-ping' : 'bg-green-500'}`}
								/>
